...
///
#define QS_PLUGIN_VERSION ( "8.3" ) /// "8.3"
...
///
/// EXECUTE THE TASK ON THE NEXT FRAME ( AS SOON AS POSSIBLE )
///
#define QS_TASK_DELAY_NEXT_FRAME ( 0.000000 ) /// 0
...
///
/// THE SIZE OF AN ARRAY OF PLAYERS
///
#define QS_PLAYERS_ARRAY_SIZE ( 1 + QS_MAX_PLAYERS ) /// 1 + QS_MAX_PLAYERS [ 33 ]
...
///
/// CS/ CZ ROUND END EXTENSION STEP
///
/// WHEN THE ROUND ENDS, DEPENDING ON HOW MANY SOUNDS/ MESSAGES ARE STILL LEFT TO BE PLAYED/ DISPLAYED, EXTENDS A BIT THE ROUND END PERIOD [ SECOND(S) ]
///
#define QS_ROUND_END_EXTENSION_STEP ( 0.050025 ) /// .050025
...
///
/// THE HATTRICK FEATURE :: ROUND END TRIGGER DELAY /// # CS/ CZ # ///
///
#define QS_HATTRICK_ROUND_END_DELAY ( 2.800000 ) /// 2.8 [[[ CS/ CZ ]]]
...
///
/// THE FLAWLESS VICTORY FEATURE :: MINIMUM TOTAL PLAYERS IN A TEAM IN ORDER TO ALLOW THE FLAWLESS VICTORY FEATURE TO BE TRIGGERED FOR THAT TEAM
///
...
///
#define QS_KNIFE_MAX_DISTANCE_UNITS ( 256 ) /// 256
...
///
/// MAXIMUM CHAT COMMAND LENGTH
///
#define QS_COMMAND_MAX_LEN ( 24 ) /// 24
...
///
/// GAME MESSAGE MAXIMUM LENGTH ( `write_string ( )` )
///
#define QS_GAME_MESSAGE_MAX_LEN ( 256 ) /// 256
...
///
#define QS_EVERYONE ( QS_INVALID_PLAYER ) /// QS_INVALID_PLAYER [ 0 ]
...
/// THE WORLDSPAWN
///
#define QS_WORLDSPAWN ( QS_MIN_ENTITY_ID ) /// QS_MIN_ENTITY_ID [ 0 ]
...
///
/// THE DEFAULT CHAT COMMAND #0 ( CAN STILL BE EDITED INTO THE INI FILE )
///
#define QS_DEF_CHAT_CMD_0 ( "!sounds" ) /// "!sounds"

///
/// THE DEFAULT CHAT COMMAND #1 ( CAN STILL BE EDITED INTO THE INI FILE )
///
#define QS_DEF_CHAT_CMD_1 ( ".sounds" ) /// ".sounds"

///
/// THE DEFAULT CHAT COMMAND #2 ( CAN STILL BE EDITED INTO THE INI FILE )
///
#define QS_DEF_CHAT_CMD_2 ( "/sounds" ) /// "/sounds"

///
/// THE DEFAULT CHAT COMMAND #3 ( CAN STILL BE EDITED INTO THE INI FILE )
///
#define QS_DEF_CHAT_CMD_3 ( ",sounds" ) /// ",sounds"
...
///
/// NEGATIVE BYTE ( FOR A RANDOM HUD MESSAGE COLOR OR MAYBE NOT USING A CUSTOM HUD MESSAGE COLOR FOR THIS EVENT )
///
#define QS_NEGATIVE_BYTE ( -1 ) /// -1
...
#define m_CBaseMonster_LastHitGroup_HL   90 /// int CBaseMonster::m_LastHitGroup
...
/// THE CHAT COMMANDS
///
static Array: g_pChatCmds = Invalid_Array;

///
/// THE CHAT COMMAND(S) AVAILABLE
///
static g_nChatCmds = 0;
...
/// CS/ CZ ROUND END EXTENSION STEP
///
static Float: g_fRoundEndExtensionStep = QS_ROUND_END_EXTENSION_STEP;
...
///
/// THE LAST MAN STANDING GUY ONLY SOUND
///
static bool: g_bTLMStandingGuyOnlySound = false;
...
//
static g_nRedEvent = QS_NEGATIVE_BYTE;
static g_nGreenEvent = QS_NEGATIVE_BYTE;
static g_nBlueEvent = QS_NEGATIVE_BYTE;

/** FLAWLESS VICTORY [ TE ( CS/ CZ TERRORIST            TEAM ) ] */
//
static g_nRedFlawlessTE = QS_NEGATIVE_BYTE;
static g_nGreenFlawlessTE = QS_NEGATIVE_BYTE;
static g_nBlueFlawlessTE = QS_NEGATIVE_BYTE;

/** FLAWLESS VICTORY [ CT ( CS/ CZ COUNTER-TERRORIST    TEAM ) ] */
//
static g_nRedFlawlessCT = QS_NEGATIVE_BYTE;
static g_nGreenFlawlessCT = QS_NEGATIVE_BYTE;
static g_nBlueFlawlessCT = QS_NEGATIVE_BYTE;

/** HATTRICK */
//
static g_nRedHattrick = QS_NEGATIVE_BYTE;
static g_nGreenHattrick = QS_NEGATIVE_BYTE;
static g_nBlueHattrick = QS_NEGATIVE_BYTE;

/** REVENGE */
//
static g_nRedRevenge = QS_NEGATIVE_BYTE;
static g_nGreenRevenge = QS_NEGATIVE_BYTE;
static g_nBlueRevenge = QS_NEGATIVE_BYTE;

/** ROUND START */
//
static g_nRedRound = QS_NEGATIVE_BYTE;
static g_nGreenRound = QS_NEGATIVE_BYTE;
static g_nBlueRound = QS_NEGATIVE_BYTE;

/** CS/ CZ THE LAST MAN STANDING [ TE ( CS/ CZ TERRORIST            TEAM ) ] */
//
static g_nRedStandingTE = QS_NEGATIVE_BYTE;
static g_nGreenStandingTE = QS_NEGATIVE_BYTE;
static g_nBlueStandingTE = QS_NEGATIVE_BYTE;

/** CS/ CZ THE LAST MAN STANDING [ CT ( CS/ CZ COUNTER-TERRORIST    TEAM ) ] */
//
static g_nRedStandingCT = QS_NEGATIVE_BYTE;
static g_nGreenStandingCT = QS_NEGATIVE_BYTE;
static g_nBlueStandingCT = QS_NEGATIVE_BYTE;

/** RAMPAGE, OWNAGE, EXCELLENT, IMPRESSIVE, ... */
//
static g_nRedStreak = QS_NEGATIVE_BYTE;
static g_nGreenStreak = QS_NEGATIVE_BYTE;
static g_nBlueStreak = QS_NEGATIVE_BYTE;
...
///
/// DEATHMSG TOLD WEAPON NAME
///
static bool: g_bDeathMsgWeapon = false;
...
static g_nGameRules = QS_INVALID_ENTITY;
...
///
static g_pnKills[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };
...
/// RESETS NEXT ROUND
///
static g_pnKillsThisRound[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };

///
/// HLTV
///
static bool: g_pbHLTV[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// BOT
///
static bool: g_pbBOT[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// IN GAME
///
static bool: g_pbInGame[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// VALID STEAM AND AUTHORIZED
///
static bool: g_pbValidSteam[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// NAME
///
static g_pszName[QS_PLAYERS_ARRAY_SIZE][QS_NAME_MAX_LEN];

///
/// STEAM
///
static g_pszSteam[QS_PLAYERS_ARRAY_SIZE][QS_STEAM_MAX_LEN];

///
/// PLAYER PREFERENCES LOADED
///
static bool: g_pbLoaded[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// PLAYER PREFERENCES EXISTING INTO THE DATABASE
///
static bool: g_pbExisting[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// PLAYER PREFERENCES ACCESS
///
static bool: g_pbAccess[QS_PLAYERS_ARRAY_SIZE] = { false, ... };

///
/// REVENGE KILL NAME STAMP
///
static g_pszRevengeStamp[QS_PLAYERS_ARRAY_SIZE][QS_NAME_MAX_LEN];

///
/// REVENGE KILL USER ID STAMP
///
static g_pnRevengeStamp[QS_PLAYERS_ARRAY_SIZE] = { QS_INVALID_USER_ID, ... };
...
///
static bool: g_pbDisabled[QS_PLAYERS_ARRAY_SIZE] = { false, ... };
...
/// SOUNDS DISABLED PER PLAYER
///
static bool: g_pbDisabled[QS_PLAYERS_ARRAY_SIZE] = { true, ... };
...
///
static g_pnUserId[QS_PLAYERS_ARRAY_SIZE] = { QS_INVALID_USER_ID, ... };

///
/// LAST KILL TIME STAMP ( GAME TIME )
///
static Float: g_pfLastKillTimeStamp[QS_PLAYERS_ARRAY_SIZE] = { 0.000000, ... };

///
/// LAST DISPLAYED MESSAGE TIME STAMP ( GAME TIME )
///
static Float: g_pfLastDisplayedMsgTimeStamp[QS_PLAYERS_ARRAY_SIZE] = { 0.000000, ... };

///
/// LAST PLAYED SOUND TIME STAMP ( GAME TIME )
///
static Float: g_pfLastPlayedSoundTimeStamp[QS_PLAYERS_ARRAY_SIZE] = { 0.000000, ... };

///
/// XSTATS [ client_death ( ) ] HIT PLACE ID STAMP FOR EVERY VICTIM
///
static g_pnPlace[QS_PLAYERS_ARRAY_SIZE] = { QS_INVALID_PLACE, ... };

///
/// XSTATS [ client_death ( ) ] WEAPON ID STAMP FOR EVERY VICTIM
///
static g_pnWeapon[QS_PLAYERS_ARRAY_SIZE] = { QS_INVALID_WEAPON, ... };

///
/// XSTATS [ client_death ( ) ] TEAM KILL BOOLEAN STAMP FOR EVERY VICTIM
///
static g_pnTeamKill[QS_PLAYERS_ARRAY_SIZE] = { QS_TEAM_KILL_NO, ... };

///
/// XSTATS [ client_death ( ) ] EXECUTION TIME STAMP FOR EVERY VICTIM
///
static Float: g_pfXStatsTimeStamp[QS_PLAYERS_ARRAY_SIZE] = { 0.000000, ... };

///
/// ACTUAL DELAYED HUD MESSAGE INDEX THAT IS DISPLAYED [0 & +]
///
static g_pnActualDelayedMessage[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };

///
/// ACTUAL DELAYED COMMAND INDEX ( SPEAK COMMAND ) THAT IS EXECUTED [0 & +]
///
static g_pnActualDelayedCommand[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };

///
/// FUTURE DELAYED HUD MESSAGE INDEX ( SPEAK COMMAND ) THAT WILL BE DISPLAYED SHORTLY [0 & +]
///
static g_pnFutureDelayedMessage[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };

///
/// FUTURE DELAYED COMMAND INDEX ( SPEAK COMMAND ) THAT WILL BE EXECUTED SHORTLY [0 & +]
///
static g_pnFutureDelayedCommand[QS_PLAYERS_ARRAY_SIZE] = { 0, ... };

///
/// EVER BEEN ALIVE
///
static bool: g_pbEverBeenAlive[QS_PLAYERS_ARRAY_SIZE] = { false, ... };
...

{
    ...
    else if (equali(szModule, "Orpheu"))
    {
        ...
        g_nGameRules = QS_INVALID_ENTITY;
...
public plugin_precache()
{
    ...
    ///
    for (nIter = QS_INVALID_PLAYER; nIter < QS_PLAYERS_ARRAY_SIZE; nIter++)
    {
    ...
    ///
    /// RETRIEVES CHAT COMMAND(S) COUNT
    ///
    g_nChatCmds = ArraySize(g_pChatCmds);
    ...
    if (!g_bOrpheuUnavailable)
    {
        ...
        {
            if (((g_bHattrick) && (g_fHattrickRoundEndSecExtension > 0.000000)) || ((g_bFlawless) && (g_fFlawlessRoundEndSecExtension > 0.000000)) ||
                (0.000000 < g_fSecDelayDisplayPlayerEvents))
            {
                g_nGameRules = QS_INVALID_ENTITY; /// THIS HAS TO BE NEGATIVE ( INVALID ) HERE, SEE BELOW

                g_bOrpheuError = true; /// THIS HAS TO BE TRUE HERE, SEE BELOW
                g_bOrpheuUnavailable = true; /// THIS HAS TO BE TRUE HERE, SEE BELOW

                new OrpheuFunction: eFunction = OrpheuGetFunction("InstallGameRules");

                /// IF THE SIGNATURE IS OUT OF DATE, THE CODE BELOW THIS LINE WILL NEVER BE EXECUTED ... AN ERROR LOG WILL BE THROWN & THE plugin_precache EXECUTION STOPS RIGHT HERE ...

                if (OrpheuInvalidFunction != eFunction)
                {
                    g_bOrpheuError = false;
                    g_bOrpheuUnavailable = false;

                    OrpheuRegisterHook(eFunction, "OnOrpheuGameRulesRevealed_POST", OrpheuHookPost);
                }
...
public OrpheuHookReturn: OnOrpheuGameRulesRevealed_POST()
{
        ...
        (0.000000 < g_fSecDelayDisplayPlayerEvents))
    {
        ...
        if (!QS_ValidEntity(g_nGameRules))
        {
            g_bOrpheuError = true;
            g_bOrpheuUnavailable = true;
...
public plugin_init()
{
    ...
    if (g_bSql)
    {
        ...
        else
        {
            ...
            log_to_file(QS_LOG_FILE_NAME, "****************************************************************************************************************");
            log_to_file(QS_LOG_FILE_NAME, "Sql Database Connection Failed.");
            log_to_file(QS_LOG_FILE_NAME, "The Following Call Failed [ SQL_MakeDbTuple ( '%s', '%s', '%s', '%s', %d ) ].",
                !QS_EmptyString(g_szSqlAddr) ? g_szSqlAddr : "N/ A", !QS_EmptyString(g_szSqlUser) ? g_szSqlUser : "N/ A",
                !QS_EmptyString(g_szSqlPassword) ? g_szSqlPassword : "N/ A", !QS_EmptyString(g_szSqlDatabase) ? g_szSqlDatabase : "N/ A", g_nSqlMaxSecondsError);
            log_to_file(QS_LOG_FILE_NAME, "Per Steam Player Preferences Will Not Be Stored In Any Database.");
        }
    }
...
public QS_HAM_On_Player_Killed_PRE(nVictim, nAttacker, nShouldGib)
{
    ...
    /// DATA
    ///
    static nWeapon = QS_INVALID_WEAPON, pnInfo[8] = { QS_INVALID_PLAYER, ... }, Float: fGameTime = 0.000000,
        Float: fGameTimeStamp = 0.000000, nVictimStamp = QS_INVALID_PLAYER;
    ...
    ///
    /// ONLY IF AVAILABLE KILLER FOR THE "QS_HAM_On_Player_Killed_PRE ( )" FORWARD
    ///
    if (QS_IsPlayer(g_nKiller) && g_pbInGame[g_nKiller])
    {
        ...
        g_nTeamKill = (QS_GetPlayerTeam(g_nKiller) == QS_GetPlayerTeam(g_nVictim)) ? QS_TEAM_KILL_YES : QS_TEAM_KILL_NO;
        ...
        {
            set_task(QS_TASK_DELAY_NEXT_FRAME, "QS_ProcessDeathHook", get_systime(0), pnInfo, charsmax(pnInfo), "", 0);
        }
    }
...
///
/// Ham_Spawn -> QS_HAM_On_Player_Spawn_POST ( nPlayer )
///
/// THE PLAYER HAS BEEN SPAWNED
///
public QS_HAM_On_Player_Spawn_POST(nPlayer)
{
    ///
    /// SANITY CHECK
    ///
...
public plugin_cfg()
{
    ...
    g_nDeathMsg = get_user_msgid("DeathMsg");
    {
        if (!QS_ValidMsg(g_nDeathMsg))
        {
            ...
            if (g_bDeathMsgOnly)
            {
                ...
                RegisterHam(Ham_Killed, "player", "QS_HAM_On_Player_Killed_PRE", 0 /** PRE = 0 */,
                    true /** IF TRUE THEN IT WILL PROVIDE SOME KIND OF SUPPORT FOR SPECIAL BOTS WITHOUT "player" CLASS NAME */); /// LATEST

#else /// defined amxclient_cmd && defined RegisterHamPlayer

                RegisterHam(Ham_Killed, "player", "QS_HAM_On_Player_Killed_PRE", 0 /** PRE = 0 */); /// OLDER
        ...
        else
        {
            ...
            ///
            register_forward(FM_MessageBegin, "QS_FM_OnMsgBegin_POST", 1 /** POST = 1 */);
            register_forward(FM_WriteByte, "QS_FM_OnWriteByte_POST", 1 /** POST = 1 */);
            register_forward(FM_WriteString, "QS_FM_OnWriteString_POST", 1 /** POST = 1 */);
            register_forward(FM_MessageEnd, "QS_FM_OnMsgEnd_POST", 1 /** POST = 1 */);
        }
    }
    ...
    RegisterHam(Ham_Spawn, "player", "QS_HAM_On_Player_Spawn_POST", 1 /** POST = 1 */,
        true /** IF TRUE THEN IT WILL PROVIDE SOME KIND OF SUPPORT FOR SPECIAL BOTS WITHOUT "player" CLASS NAME */); /// LATEST
...
///
public client_remove(nPlayer, bool: bDrop, const szMsg[])
{
    ///
    /// REDIRECT THE CALL
    ///
...
public QS_ON_PLAYER_DISCONNECTED
{
    ...
    ///
    /// CONSIDER THAT NEVER BEEN ALIVE
    ///
    g_pbEverBeenAlive[nPlayer] = false;
...
public client_command(nPlayer)
{
    ...
    /// DATA
    ///
    static nIter = 0, nLen = 0, szArg[QS_COMMAND_MAX_LEN] = { EOS, ... }, szCmd[QS_COMMAND_MAX_LEN] = { EOS, ... },
        szQuery[QS_SQL_QUERY_MAX_LEN] = { EOS, ... }, szUserId[QS_NUMBER_MAX_LEN] = { EOS, ... };
    ...
    if (g_pbInGame[nPlayer] && !g_pbBOT[nPlayer] && !g_pbHLTV[nPlayer])
    {
        ...
        /// CHECKS THE ARGUMENT
        ///
        for (nIter = 0; nIter < g_nChatCmds; nIter++)
        {
            nLen = ArrayGetString(g_pChatCmds, nIter, szCmd, charsmax(szCmd));

            if (nLen < 1)
            { /// OLDER AMXX EDITIONS COMPATIBILITY ...
                if (QS_EmptyString(szCmd))
                {
                    continue;
                }
            }
            ...
            ///
            if (!equali(szArg, szCmd, ((nLen < 1) ? (strlen(szCmd)) : (nLen))))
            {
                continue;
            }
            ...
            ///
            /// SHOW THAT THEY TYPED '/sounds' OTHERWISE
            ///
            return PLUGIN_CONTINUE;
        }
    }
...
///
public QS_DelayedPlayerMessage(pnInfo[], nTaskIndex)
{
        ...
        if (nFutureTaskIndex == g_pnActualDelayedMessage[nPlayer])
        {
            if (!bHidden)
            {
                if (g_pnHudMsgObj[QS_HUD_EVENT] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_EVENT_Y_POS_DOD : QS_EVENT_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                        QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
                ...
                else if (g_pnHudMsgObj[QS_HUD_REVENGE] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_REVENGE_Y_POS_DOD : QS_REVENGE_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                        QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }

                else if (g_pnHudMsgObj[QS_HUD_STANDING] == nHudObj)
                {
                    if (g_bTLMStandingDone_CT && g_bTLMStandingDone_TE)
                    {
                        QS_HudMsgColor(QS_HUD_STANDING, g_nTLMStandingGuyTeam);
                    }

                    else if (g_bTLMStandingDone_CT)
                    {
                        QS_HudMsgColor(QS_HUD_STANDING, QS_CSCZ_TEAM_CT);
                    }

                    else if (g_bTLMStandingDone_TE)
                    {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, QS_STANDING_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                        QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
                ...
                else if (g_pnHudMsgObj[QS_HUD_STREAK] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_STREAK_Y_POS_DOD : QS_STREAK_Y_POS, QS_HUD_MSG_EFFECTS,
                        QS_HUD_MSG_EFFECTS_TIME, QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
                ...
                else if (g_pnHudMsgObj[QS_HUD_FLAWLESS] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, QS_FLAWLESS_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                        QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
                ...
                else if (g_pnHudMsgObj[QS_HUD_HATTRICK] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_HATTRICK_Y_POS_DOD : QS_HATTRICK_Y_POS, QS_HUD_MSG_EFFECTS,
                        QS_HUD_MSG_EFFECTS_TIME, QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
                ...
                else if (g_pnHudMsgObj[QS_HUD_ROUND] == nHudObj)
                {
                    ...
                    QS_MakeRGBA(pnColor);
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_ROUND_Y_POS_DOD : QS_ROUND_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                        QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                }
...
///
public QS_DelayedPlayerCommand(pnInfo[], nTaskIndex)
{
...
///
public QS_DisplayPlayerInfo(szParam[], nTaskIndex)
{
    ...
    ///
    nUserId = (nTaskIndex - str_to_num(szParam));
...
///
public QS_PrepareManStanding(nTaskIndex)
{
    static nTheAliveGuyTE = QS_INVALID_PLAYER, nTheAliveGuyCT = QS_INVALID_PLAYER;

    if (((QS_GetTeamTotalAlive(QS_CSCZ_TEAM_TE, nTheAliveGuyTE) == 1) && (QS_ActivePlayersNum(false, QS_CSCZ_TEAM_TE) > 0)) ||
        ((QS_GetTeamTotalAlive(QS_CSCZ_TEAM_CT, nTheAliveGuyCT) == 1) && (QS_ActivePlayersNum(false, QS_CSCZ_TEAM_CT) > 0)))
    {
        set_task(QS_TASK_DELAY_NEXT_FRAME, "QS_PerformManStanding", get_systime(0), "", 0, "", 0);
    }
...
///
public QS_PerformManStanding(nTaskIndex)
{
    ...
    if ((g_bTLMStandingDone_TE == false) && (nTEs == 1) && (nCTs > 0) &&
        (QS_ActivePlayersNum(false, QS_CSCZ_TEAM_TE) > 0) && (QS_CanTriggerTLMStandingTeam(QS_CSCZ_TEAM_TE)))
    {
        ...
        {
            if (!(g_pbInGame[nPlayer]) || (g_pbBOT[nPlayer]) || (g_pbHLTV[nPlayer]) || (QS_GetPlayerTeam(nPlayer) != QS_CSCZ_TEAM_TE))
            {
                continue;
            }
            ...
            else
            {
                if (g_bTLMStandingTeamMsg)
                {
                    QS_ShowHudMsg(nPlayer, g_pnHudMsgObj[QS_HUD_STANDING], g_szTLMStandingTeamMsg,
                        g_pszName[nTEGuy], ArrayGetStringHandle(g_pTLMStandingWords, random_num(0, g_nTLMStandingWordsSize - 1)));
                }
                ...
                if (!g_bTLMStandingGuyOnlySound)
                {
                    QS_ClientCmd(nPlayer, "SPK \"%a\"", ArrayGetStringHandle(g_pTLMStanding, random_num(0, g_nTLMStandingSize - 1)));
                }
            }
        }
    }
        ...
        (QS_ActivePlayersNum(false, QS_CSCZ_TEAM_CT) > 0) && (QS_CanTriggerTLMStandingTeam(QS_CSCZ_TEAM_CT)))
    {
        ...
        for (nPlayer = QS_MIN_PLAYER; nPlayer <= g_nMaxPlayers; nPlayer++)
        {
            if (!(g_pbInGame[nPlayer]) || (g_pbBOT[nPlayer]) || (g_pbHLTV[nPlayer]) || (QS_GetPlayerTeam(nPlayer) != QS_CSCZ_TEAM_CT))
            {
                continue;
            }
            ...
            else
            {
                ...
                {
                    QS_ShowHudMsg(nPlayer, g_pnHudMsgObj[QS_HUD_STANDING], g_szTLMStandingTeamMsg,
                        g_pszName[nCTGuy], ArrayGetStringHandle(g_pTLMStandingWords, random_num(0, g_nTLMStandingWordsSize - 1)));
                }
...
public QS_ExtendTheRoundEndPeriod(nTaskIndex)
{
    ...
    if (g_bGameDataError)
    {
        ...
        if ((g_bOrpheuUnavailable) || (g_bOrpheuError) || !(QS_ValidEntity(g_nGameRules)))
        {
            ...
            fExtension = (float(nMaximumDelayedPlayerEvents) * (g_fSecDelayDisplayPlayerEvents + g_fRoundEndExtensionStep));
...
/// PREPARES THE ROUND START EVENT
///
public QS_RoundStart(nTaskIndex)
{
    ///
    /// DATA
    ///
...
/// PREPARES HATTRICK
///
public QS_Hattrick(nTaskIndex)
{
    ///
    /// DATA
    ///
    static nLeader = QS_INVALID_PLAYER, nTeam = QS_INVALID_TEAM, nTotal = 0, nPlayer = QS_INVALID_PLAYER, nIter = 0, pnColor[4] = { QS_MIN_BYTE, ... },
        pnPlayers[QS_MAX_PLAYERS /** `QS_MAX_PLAYERS` REQUIRED FOR 'get_players ( )', NOT `QS_PLAYERS_ARRAY_SIZE` */] = { QS_INVALID_PLAYER, ... };
        ...
        if (g_pnKillsThisRound[nLeader] >= g_nMinKillsForHattrick)
        {
            ...
            if (g_bHattrickMsg)
            {
                QS_HudMsgColor(QS_HUD_HATTRICK, QS_INVALID_TEAM);
                {
                    QS_MakeRGBA(pnColor);
                    {
                        QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_HATTRICK_Y_POS_DOD : QS_HATTRICK_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                            QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                    }
                }
            ...
            if (g_bHattrickToAll)
            {
                ...
                if (g_bHattrickMsgTalk)
                {
                    switch (g_bColors)
                    {
                        case true:
                        {
                            nTeam = QS_GetPlayerTeam(nLeader);
                            {
                                switch (nTeam)
                                {
                                    case QS_CSCZ_TEAM_CT:
                                    {
                                        QS_CSCZColored(QS_EVERYONE, QS_CC_ST_X03_BLUE, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }

                                    case QS_CSCZ_TEAM_TE:
                                    {
                                        QS_CSCZColored(QS_EVERYONE, QS_CC_ST_X03_RED, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }

                                    default:
                                    {
                                        QS_CSCZColored(QS_EVERYONE, QS_CC_ST_X03_GREY, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }
                                }
                            }
                        ...
                        default:
                        {
                            client_print(QS_EVERYONE, print_chat, g_szPhrHattrickVictimsDef, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                        }
                    }
                }
                        ...
                        case true:
                        {
                            nTeam = QS_GetPlayerTeam(nLeader);
                            {
                                switch (nTeam)
                                {
                                    case QS_CSCZ_TEAM_CT:
                                    {
                                        QS_CSCZColoredFiltered(QS_EVERYONE, QS_CC_ST_X03_BLUE, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }

                                    case QS_CSCZ_TEAM_TE:
                                    {
                                        QS_CSCZColoredFiltered(QS_EVERYONE, QS_CC_ST_X03_RED, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }

                                    default:
                                    {
                                        QS_CSCZColoredFiltered(QS_EVERYONE, QS_CC_ST_X03_GREY, g_szPhrHattrickVictims, g_pszName[nLeader], g_pnKillsThisRound[nLeader],
                                            g_pnKillsThisRound[nLeader] == 1 ? "" : g_szPhrHattrickVictimsPluralSuf);
                                    }
                                }
                            }
...
public QS_Flawless(nTaskIndex)
{
    ...
    if ((g_nFlawlessTESize >= g_nFlawlessMinimumReqPlayers) && (g_nFlawlessTESize == g_nFlawlessAliveTESize) && (g_nFlawlessCTSize))
    {
        ...
        QS_HudMsgColor(QS_HUD_FLAWLESS, QS_CSCZ_TEAM_TE);
        {
            QS_MakeRGBA(pnColor);
            {
                QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, QS_FLAWLESS_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME, QS_HUD_MSG_HOLD_TIME,
                    QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
            }
        }
        ...
        if (g_fFlawlessRoundEndSecExtension > 0.000000)
        {
            if (g_bCSCZ)
            {
                ...
                else if ((QS_ValidEntity(g_nGameRules)) && !(g_bOrpheuError) && !(g_bOrpheuUnavailable))
                {
                    fOriginal = Float: OrpheuMemoryGetAtAddress(g_nGameRules, "m_fTeamCount");
...
///
public QS_FM_OnWriteByte_POST(nByte)
{
    ///
    /// OUR DEATHMSG
    ///
...
/// pfnWriteString ( )
///
/// FIRED WHEN A STRING IS BEING WRITTEN
///
public QS_FM_OnWriteString_POST(szBuffer[])
{ /// DAY OF DEFEAT ( DOD ) HAS NO 'WriteString()' FOR `DeathMsg`
                ...
                switch (g_bCSCZ)
                {
                    ...
                    { /// VALVE ( FOR EXAMPLE )
                        ...
                        else
                        {
                            ...
                            {
                                add(szWeapon, charsmax(szWeapon), "handgrenade");
                            }
                    ...
                    } /// IN VALVE, A STRING LIKE "teammate" CAN SHOW UP

                    /**
                     * VALVE COMES UP WITH "teammate" WEAPON FOR TEAM KILLS ( IF 'mp_teamplay' ON )
                     */
...
///
public QS_FM_OnMsgEnd_POST()
{
    static Float: fGameTime = 0.000000, Float: fGameTimeStamp = 0.000000, nVictimStamp = QS_INVALID_PLAYER, nKillerStamp = QS_INVALID_PLAYER,
        pnInfo[8] = { QS_INVALID_PLAYER, ... }, nWeapon = QS_INVALID_WEAPON, bool: bDeathMsgWeapon = false;
        ...
        switch ((nVictimStamp == g_nVictim) && (nKillerStamp == g_nKiller))
        {
            case false:
            {
                ...
                if (QS_IsPlayer(g_nVictim) && g_pbInGame[g_nVictim])
                {
                    ...
                    if (g_bHL)
                    {
                        g_nPlace = get_pdata_int(g_nVictim, m_CBaseMonster_LastHitGroup_HL);
                    }
                ...
                {
                    pnInfo[2] = !bDeathMsgWeapon ? QS_INVALID_WEAPON : g_nWeapon;
...
public QS_PickThreadedQueryHandler(nFailState, Handle: pQuery, szError[], nErrorCode, szCustomUserData[], nCustomUserDataSize /** != LENGTH */, Float: fQueueTimeSeconds)
{
    ...
    /// DATA
    ///
    static nUserId = QS_INVALID_USER_ID, nPlayer = QS_INVALID_PLAYER, szQuery[QS_SQL_QUERY_MAX_LEN] = { EOS, ... }, bWasDisabled = false,
        nSysTime = 0, szParam[QS_NUMBER_MAX_LEN] = { EOS, ... }, Float: fErrorStamp = 0.000000, Float: fGameTime = 0.000000;
                        ...
                        if (pQuery != Empty_Handle)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if (!g_bSqlFullSteam)
                                    {
                                        formatex(szQuery, charsmax(szQuery), "INSERT INTO aqs_enabled_fast (aqs_steam, aqs_option) VALUES ('%s', %d);",
                                            g_pszSteam[nPlayer], g_pbDisabled[nPlayer] ? 0 : 1);
                                    }

                                    else
                                    {
                                        formatex(szQuery, charsmax(szQuery), "INSERT INTO aqs_enabled_full (aqs_steam, aqs_option) VALUES ('%s', %d);",
                                            g_pszSteam[nPlayer], g_pbDisabled[nPlayer] ? 0 : 1);
                                    }
                                }
...
///
public QS_DisableSql(nTaskIndex)
{
    if (g_bSql)
    {
        if (Empty_Handle != g_pSqlDb)
...
///
public QS_RetryPick(szParam[], nTaskIndex)
{
...
///
public QS_RetryStore(szParam[], nTaskIndex)
{
    static nPlayer = QS_INVALID_PLAYER, nUserId = QS_INVALID_USER_ID, szQuery[QS_SQL_QUERY_MAX_LEN] = { EOS, ... }, szUserId[QS_NUMBER_MAX_LEN] = { EOS, ... };
...
///
public QS_ProcessDeathMsg(pnInfo[], nTaskIndex)
{
    ///
    ...
    {
        case false:
        { /// DOD ALREADY USES `m_CBaseMonster_LastHitGroup_DoD` AND HL ALREADY USES `m_CBaseMonster_LastHitGroup_HL`
            if (!g_bDOD && !g_bHL && (g_bDeathMsgOnly || bReExec || !QS_ValidPlace(g_pnPlace[nVictim])))
            {
            ...
            { /// HL, UNLIKE DOD, COMES WITH NO client_death()
                g_pnPlace[nVictim] = nOrigPlace;
            }
        }
    ...
    ///
    if (bDeathMsgWpn) /// THIS CONDITION CAN ONLY BE TRUE IN CS/ CZ AND HL AT THE MOMENT
...
///
public QS_ProcessDeathHook(pnInfo[], nTaskIndex)
{
    ///
...
static bool: QS_ProcessPlayerDeath(nKiller, &nVictim, &nWeapon, &nPlace, &nTeamKill)
{
    ...
    ///
    if (!g_pbInGame[nKiller]) /** THE KILLER IS ALWAYS IN GAME WHEN THE QS_ProcessPlayerDeath ( ) IS TRIGGERED BY THE QS_HAM_On_Player_Killed_PRE ( ) */
    {
        ...
        else
        {
            ...
            if ((g_bFBlood) && (++g_nFBlood == 1))
            {
                ...
                else
                {
                    ...
                    /// PREPARES THE FIRST BLOOD HUD MESSAGE @ QS_EVENT_Y_POS
                    ///
                    QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_EVENT_Y_POS_DOD : QS_EVENT_Y_POS, QS_HUD_MSG_EFFECTS,
                        QS_HUD_MSG_EFFECTS_TIME, QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
    ...
    else
    {
        ...
        if ((g_bRevenge) && (!(bExecutedTeamKill) || (g_bRevengeIfTeamKill)) &&
            (g_pnUserId[nVictim] == g_pnRevengeStamp[nKiller]) && (equali(g_pszName[nVictim], g_pszRevengeStamp[nKiller])))
        {
            ///
            ...
            ///
            QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_REVENGE_Y_POS_DOD : QS_REVENGE_Y_POS, QS_HUD_MSG_EFFECTS,
                QS_HUD_MSG_EFFECTS_TIME, QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
        ...
        if (g_bKStreak)
        {
            ...
            if (nMatchCount > 0)
            {
                nIter = nMatches[random_num(0, nMatchCount - 1)];
...
static bool: QS_LoadSettings()
{
    ...
    while (!feof(nFile))
    {
        ...
        else if (equali(szKey, "CHAT COMMANDS") || equali(szKey, "CHAT CMDS")) /** COMPATIBILITY */
        {
            if ((QS_EmptyString(szVal)) && (0 == ArraySize(g_pChatCmds)))
            {
                ArrayPushString(g_pChatCmds, QS_DEF_CHAT_CMD_0);
                ArrayPushString(g_pChatCmds, QS_DEF_CHAT_CMD_1);
                ArrayPushString(g_pChatCmds, QS_DEF_CHAT_CMD_2);
                ArrayPushString(g_pChatCmds, QS_DEF_CHAT_CMD_3);
            }
            ...
            while (!(QS_EmptyString(szVal)) && (strtok2(szVal, szKey, charsmax(szKey), szVal, charsmax(szVal), ';', 1) >= -1))
            {
                ...
                {
                    ArrayPushString(g_pChatCmds, szKey);
                }
            }
        }
        ...
        else if (equali(szKey, "TEAM RETRIEVAL") || equali(szKey, "TEAM RETRIEVAL METHOD")) /** COMPATIBILITY */
        {
            g_nTeamRetrieval = clamp(abs(str_to_num(szVal)), QS_TEAM_RETRIEVAL_DEFAULT, QS_TEAM_RETRIEVAL_LAST_ENTRY);
        }
        ...
        else if (equali(szKey, "QS_DISPLAY_PLAYER_EVENT_DELAY") || equali(szKey, "QS_DISPLAY_PLAYER_DELAY") ||
            equali(szKey, "QS_DISPLAY_DELAY") || equali(szKey, "QS_DISPLAY_EVENT_DELAY")) /** COMPATIBILITY */
        {
        ...
        else if (equali(szKey, "TLMSTANDING GUY ONLY") || equali(szKey, "TLMSTANDING ONLY GUY")) /** COMPATIBILITY */
        {
            g_bTLMStandingGuyOnlySound = bool: str_to_num(szVal);
        }
        ...
        else if (equali(szKey, "HUD RGB EVENT"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedEvent = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenEvent = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueEvent = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedEvent = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenEvent = QS_NEGATIVE_BYTE;
                    g_nBlueEvent = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedEvent = QS_NEGATIVE_BYTE;
                    g_nGreenEvent = QS_NEGATIVE_BYTE;
                    g_nBlueEvent = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedEvent = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenEvent = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueEvent = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB STREAK"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedStreak = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStreak = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStreak = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedStreak = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStreak = QS_NEGATIVE_BYTE;
                    g_nBlueStreak = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedStreak = QS_NEGATIVE_BYTE;
                    g_nGreenStreak = QS_NEGATIVE_BYTE;
                    g_nBlueStreak = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedStreak = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStreak = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStreak = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB REVENGE"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedRevenge = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRevenge = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueRevenge = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedRevenge = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRevenge = QS_NEGATIVE_BYTE;
                    g_nBlueRevenge = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedRevenge = QS_NEGATIVE_BYTE;
                    g_nGreenRevenge = QS_NEGATIVE_BYTE;
                    g_nBlueRevenge = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedRevenge = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRevenge = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueRevenge = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB ROUND"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedRound = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRound = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueRound = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedRound = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRound = QS_NEGATIVE_BYTE;
                    g_nBlueRound = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedRound = QS_NEGATIVE_BYTE;
                    g_nGreenRound = QS_NEGATIVE_BYTE;
                    g_nBlueRound = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedRound = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenRound = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueRound = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB FLAWLESS TE"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedFlawlessTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessTE = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueFlawlessTE = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedFlawlessTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessTE = QS_NEGATIVE_BYTE;
                    g_nBlueFlawlessTE = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedFlawlessTE = QS_NEGATIVE_BYTE;
                    g_nGreenFlawlessTE = QS_NEGATIVE_BYTE;
                    g_nBlueFlawlessTE = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedFlawlessTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessTE = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueFlawlessTE = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB FLAWLESS CT"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedFlawlessCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessCT = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueFlawlessCT = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedFlawlessCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessCT = QS_NEGATIVE_BYTE;
                    g_nBlueFlawlessCT = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedFlawlessCT = QS_NEGATIVE_BYTE;
                    g_nGreenFlawlessCT = QS_NEGATIVE_BYTE;
                    g_nBlueFlawlessCT = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedFlawlessCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenFlawlessCT = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueFlawlessCT = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB HATTRICK"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedHattrick = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenHattrick = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueHattrick = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedHattrick = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenHattrick = QS_NEGATIVE_BYTE;
                    g_nBlueHattrick = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedHattrick = QS_NEGATIVE_BYTE;
                    g_nGreenHattrick = QS_NEGATIVE_BYTE;
                    g_nBlueHattrick = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedHattrick = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenHattrick = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueHattrick = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB STANDING TE"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedStandingTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingTE = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStandingTE = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedStandingTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingTE = QS_NEGATIVE_BYTE;
                    g_nBlueStandingTE = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedStandingTE = QS_NEGATIVE_BYTE;
                    g_nGreenStandingTE = QS_NEGATIVE_BYTE;
                    g_nBlueStandingTE = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedStandingTE = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingTE = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStandingTE = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
        ...
        else if (equali(szKey, "HUD RGB STANDING CT"))
        {
            switch (parse(szVal, szRed, charsmax(szRed), szGreen, charsmax(szGreen), szBlue, charsmax(szBlue)))
            {
                case 2:
                {
                    g_nRedStandingCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingCT = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStandingCT = QS_NEGATIVE_BYTE;
                }

                case 1:
                {
                    g_nRedStandingCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingCT = QS_NEGATIVE_BYTE;
                    g_nBlueStandingCT = QS_NEGATIVE_BYTE;
                }

                case 0:
                {
                    g_nRedStandingCT = QS_NEGATIVE_BYTE;
                    g_nGreenStandingCT = QS_NEGATIVE_BYTE;
                    g_nBlueStandingCT = QS_NEGATIVE_BYTE;
                }
                ...
                default:
                {
                    g_nRedStandingCT = (('_' == szRed[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szRed)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nGreenStandingCT = (('_' == szGreen[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szGreen)), QS_MIN_BYTE, QS_MAX_BYTE)));
                    g_nBlueStandingCT = (('_' == szBlue[0]) ? (QS_NEGATIVE_BYTE) : (clamp(abs(str_to_num(szBlue)), QS_MIN_BYTE, QS_MAX_BYTE)));
                }
            }
        }
                ...
                if ((parse(szVal, szDummy, charsmax(szDummy), szType, charsmax(szType)) > 1) || !(QS_EmptyString(szType)))
                {
                    trim(szType);
                    {
                        if (equali(szType, "REQUIREDKILLS"))
                        {
                            ...
                            parse(szVal, szDummy, charsmax(szDummy), szDummy, charsmax(szDummy),
                                szReqKills, charsmax(szReqKills), szDummy, charsmax(szDummy), szSnd, charsmax(szSnd));
                            {
                                trim(szReqKills);
                                trim(szSnd);
                            }
...
static bool: QS_DisplayKStreak(&nKiller, szMsg[], szSnd[])
{
    ...
    if (!QS_EmptyString(szMsg))
    {
        QS_HudMsgColor(QS_HUD_STREAK, QS_INVALID_TEAM);
        {
            QS_MakeRGBA(pnColor);
            {
                QS_SetHudMsg(g_nRed, g_nGreen, g_nBlue, QS_HUD_MSG_X_POS, g_bDOD ? QS_STREAK_Y_POS_DOD : QS_STREAK_Y_POS, QS_HUD_MSG_EFFECTS, QS_HUD_MSG_EFFECTS_TIME,
                    QS_HUD_MSG_HOLD_TIME, QS_HUD_MSG_FADE_IN_TIME, QS_HUD_MSG_FADE_OUT_TIME, -1, QS_HUD_MSG_ALPHA_AMOUNT, pnColor);
                {
...
///
static QS_ActivePlayersNum(bool: bAlive, nTeam)
{
    ///
    /// PLAYERS COUNT
    ///
    ...
    for (nTotal = 0, nPlayer = QS_MIN_PLAYER; nPlayer <= g_nMaxPlayers; nPlayer++)
    {
        ...
        ///
        if ((g_pbInGame[nPlayer]) && (!(g_pbHLTV[nPlayer])) && ((nTeam == QS_INVALID_TEAM) || (QS_GetPlayerTeam(nPlayer) == nTeam)) && (bAlive == (bool: is_user_alive(nPlayer))))
        {
            ///
            /// TOTAL = TOTAL + 1
            ///
...
///
/// RETURNS TRUE IF IN THAT TEAM THERE IS AT LEAST ONE DEAD PLAYER WHICH EVER BEEN ALIVE
///
/// nTeam [ QS_INVALID_TEAM/ 0/ 1/ 2/ 3 ]
///
static bool: QS_CanTriggerTLMStandingTeam(nTeam)
{
    ///
    /// DATA
    ///
    ...
    {
        ///
        /// IN GAME, EVER BEEN ALIVE, NOT HLTV, IN A SPECIFIED TEAM AND DEAD
        ///
        if ((g_pbInGame[nPlayer]) && (g_pbEverBeenAlive[nPlayer]) && (!(g_pbHLTV[nPlayer])) && (QS_GetPlayerTeam(nPlayer) == nTeam) && (false == (bool: is_user_alive(nPlayer))))
        {
            ///
            /// YES
            ///
...
static bool: QS_ShowHudMsg(nTo, &nObj, szRules[], any: ...)
{
    ...
    static szBuffer[QS_HUD_MSG_MAX_LEN] = { EOS, ... }, nPlayer = QS_INVALID_PLAYER, bool: bIsPlayer = false,
        Float: fTheGameTime = 0.000000, pnInfo[4 + QS_HUD_MSG_MAX_LEN] = { 0, ... }, nSysTime = 0;
...
static bool: QS_ClientCmd(nTo, szRules[], any: ...)
{
    ...
    /// ARGUMENT FORMAT
    ///
    static szBuffer[QS_SND_MAX_LEN + 8 /** SPEAK "" */] = { EOS, ... }, nPlayer = QS_INVALID_PLAYER, bool: bIsPlayer = false,
        Float: fTheGameTime = 0.000000, pnInfo[3 + QS_SND_MAX_LEN] = { 0, ... }, nSysTime = 0;
...
/// HALF LIFE RUNNING
///
static bool: QS_HLRunning()
{
    QS_CheckMod();

    return bool: equali(g_szMod, "VALVE", 5);
}
...
///
static QS_GetTeamTotalAlive(nTeam, &nPlayer) /// CSTRIKE AND CZERO ONLY
{
    static pnPlayers[QS_MAX_PLAYERS /** `QS_MAX_PLAYERS` REQUIRED FOR 'get_players ( )', NOT `QS_PLAYERS_ARRAY_SIZE` */] = { QS_INVALID_PLAYER, ... }, nTotal = 0;

    /// aeh = EXCLUDE DEAD [a], EXCLUDE HLTV PROXIES [h] & MATCH WITH TEAM NAME [e]
    ///
...
static bool: QS_HudMsgColor(nHudObj, nTeam)
{
    ...
    switch (nHudObj)
    {
        case QS_HUD_EVENT:
        {
            if (g_nRedEvent > QS_NEGATIVE_BYTE)
            {
                g_nRed = g_nRedEvent;
            }

            if (g_nGreenEvent > QS_NEGATIVE_BYTE)
            {
                g_nGreen = g_nGreenEvent;
            }

            if (g_nBlueEvent > QS_NEGATIVE_BYTE)
            {
        ...
        case QS_HUD_FLAWLESS:
        {
            if (QS_ValidTeam(nTeam))
            {
                switch (nTeam)
                {
                    case QS_CSCZ_TEAM_TE:
                    {
                        if (g_nRedFlawlessTE > QS_NEGATIVE_BYTE)
                        {
                            g_nRed = g_nRedFlawlessTE;
                        }

                        if (g_nGreenFlawlessTE > QS_NEGATIVE_BYTE)
                        {
                            g_nGreen = g_nGreenFlawlessTE;
                        }

                        if (g_nBlueFlawlessTE > QS_NEGATIVE_BYTE)
                        {
                    ...
                    case QS_CSCZ_TEAM_CT:
                    {
                        if (g_nRedFlawlessCT > QS_NEGATIVE_BYTE)
                        {
                            g_nRed = g_nRedFlawlessCT;
                        }

                        if (g_nGreenFlawlessCT > QS_NEGATIVE_BYTE)
                        {
                            g_nGreen = g_nGreenFlawlessCT;
                        }

                        if (g_nBlueFlawlessCT > QS_NEGATIVE_BYTE)
                        {
        ...
        {
            if (g_nRedHattrick > QS_NEGATIVE_BYTE)
            {
                g_nRed = g_nRedHattrick;
            }

            if (g_nGreenHattrick > QS_NEGATIVE_BYTE)
            {
                g_nGreen = g_nGreenHattrick;
            }

            if (g_nBlueHattrick > QS_NEGATIVE_BYTE)
            {
        ...
        case QS_HUD_REVENGE:
        {
            if (g_nRedRevenge > QS_NEGATIVE_BYTE)
            {
                g_nRed = g_nRedRevenge;
            }

            if (g_nGreenRevenge > QS_NEGATIVE_BYTE)
            {
                g_nGreen = g_nGreenRevenge;
            }

            if (g_nBlueRevenge > QS_NEGATIVE_BYTE)
            {
        ...
        case QS_HUD_ROUND:
        {
            if (g_nRedRound > QS_NEGATIVE_BYTE)
            {
                g_nRed = g_nRedRound;
            }

            if (g_nGreenRound > QS_NEGATIVE_BYTE)
            {
                g_nGreen = g_nGreenRound;
            }

            if (g_nBlueRound > QS_NEGATIVE_BYTE)
            {
        ...
        case QS_HUD_STANDING:
        {
            if (QS_ValidTeam(nTeam))
            {
                switch (nTeam)
                {
                    case QS_CSCZ_TEAM_TE:
                    {
                        if (g_nRedStandingTE > QS_NEGATIVE_BYTE)
                        {
                            g_nRed = g_nRedStandingTE;
                        }

                        if (g_nGreenStandingTE > QS_NEGATIVE_BYTE)
                        {
                            g_nGreen = g_nGreenStandingTE;
                        }

                        if (g_nBlueStandingTE > QS_NEGATIVE_BYTE)
                        {
                    ...
                    case QS_CSCZ_TEAM_CT:
                    {
                        if (g_nRedStandingCT > QS_NEGATIVE_BYTE)
                        {
                            g_nRed = g_nRedStandingCT;
                        }

                        if (g_nGreenStandingCT > QS_NEGATIVE_BYTE)
                        {
                            g_nGreen = g_nGreenStandingCT;
                        }

                        if (g_nBlueStandingCT > QS_NEGATIVE_BYTE)
                        {
        ...
        case QS_HUD_STREAK:
        {
            if (g_nRedStreak > QS_NEGATIVE_BYTE)
            {
                g_nRed = g_nRedStreak;
            }

            if (g_nGreenStreak > QS_NEGATIVE_BYTE)
            {
                g_nGreen = g_nGreenStreak;
            }

            if (g_nBlueStreak > QS_NEGATIVE_BYTE)
            {
...
{
    static szMsg[QS_GAME_MESSAGE_MAX_LEN] = { EOS, ... }, nTotal = 0, nIter = 0, nOther = QS_INVALID_PLAYER, bool: bSuccess = false,
        pnPlayers[QS_MAX_PLAYERS /** `QS_MAX_PLAYERS` REQUIRED FOR 'get_players ( )', NOT `QS_PLAYERS_ARRAY_SIZE` */] = { QS_INVALID_PLAYER, ... };
...
/// SETS A HUD MESSAGE
///
static bool: QS_SetHudMsg(&nRed, &nGreen, &nBlue, Float: fX, Float: fY, nEffects, Float: fEffectsTime,
    Float: fHoldTime, Float: fFadeInTime, Float: fFadeOutTime, nChannel, nAlpha, pnColor[4])
{
...
/// IS THE ENTITY VALID
///
static bool: QS_ValidEntity(&nEntity)
{
    return (nEntity > QS_INVALID_ENTITY);
}
...
///
/// THE DISTANCE IN UNITS BETWEEN TWO PLAYERS
///
static QS_TheDistanceBetweenTwoPlayers(&nVictim, &nKiller)
{
    static pnVictimOrigin[3] = { 0, ... }, pnKillerOrigin[3] = { 0, ... };

#if defined Origin_Client

    if (!get_user_origin(nVictim, pnVictimOrigin, Origin_Client))
    {
        return 0;
    }

    if (!get_user_origin(nKiller, pnKillerOrigin, Origin_Client))
    {
        return 0;
    }

#else /// defined Origin_Client

    if (!get_user_origin(nVictim, pnVictimOrigin))
    {
        return 0;
    }

    if (!get_user_origin(nKiller, pnKillerOrigin))
    {
        return 0;
    }
...
///
/// READS A PLAYER'S TEAM
///
static QS_GetPlayerTeam(&nPlayer)
{
    static szTheTeamName[QS_TEAM_MAX_LEN] = { EOS, ... };

    if (QS_IsPlayer(nPlayer))
    {
        switch (g_nTeamRetrieval)
        {
            case QS_TEAM_RETRIEVAL_FAKE_META:
            {
